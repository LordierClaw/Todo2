package me.lordierclaw.todo2.screen.mine
...
class MineViewModel(private val repositoryBuilder: IRepositoryBuilder): ViewModel() {
    ...
    private fun observeTaskStats(lifecycleOwner: LifecycleOwner) {
        repositoryBuilder.taskRepository.getCategoryCounts().observe(lifecycleOwner) { list ->
            ...
            val sortedList: MutableList<CategoryCount> = list.sortedByDescending { it.taskCount }.toMutableList()
            for (i in 0..4) {
                if (i == sortedList.size) {
                    _taskPieEntries.value = entries
                    return@observe
                } else {
                    entries.add(PieEntry(sortedList[i].taskCount.toFloat(), sortedList[i].categoryName))
                }
            }
            var otherCount = 0
    ...
    private fun observeTaskInNext7Days(lifecycleOwner: LifecycleOwner) {
        ...
        repositoryBuilder.taskRepository.getAllTaskInTimeRange(startTime, endTime).observe(lifecycleOwner) {
